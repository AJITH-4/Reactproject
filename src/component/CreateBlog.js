...
function CreateBlog() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      imageUrl:Yup.string().matches(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g,'Must be url').required("Required"),
      description:Yup.string().min(100,"100 Character Expectd").max(400,"Too Long").required ("Required")
    }),
    onSubmit:values=>{
